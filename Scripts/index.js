...
require([
  ...
  */
  ...
  function createTable (graphic, fieldsToInclude, feetInchesFields) {
    ...
    var td
    var value
    var o = graphic.attributes
    var aliasDict = createFieldAliasDictionary(graphic.getLayer())
  ...
  var infoTemplate = new InfoTemplate('${crossing_description}', toHtmlContent) // eslint-disable-line no-template-curly-in-string
  ...
  // Create Geocoder
  new Geocoder({ // eslint-disable-line no-new
    map: map,
    autoComplete: true,
  ...
  function routeLocationToGraphic (routeLocation) {
    ...
    var graphic = {
      ...
      infoTemplate: {
        ...
        content: '<dl><dt>Route</dt><dd>${Route}</dd><dt>Milepost</dt><dd>${Srmp}</dd></dl>' // eslint-disable-line no-template-curly-in-string
      }
    }
  ...
  map.on('click', function (evt) {
    var mapPoint
    var graphicTagNames = /(?:(?:circle)|(?:path))/i
